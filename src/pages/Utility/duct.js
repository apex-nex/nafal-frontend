...
import { ductContent } from "../../data";
import { ductContentArabic } from "../../data/indexArabic";

const Duct = () => {
  const { isArabic } = useAuth()
  const data = isArabic ? ductContent : ductContentArabic
            ...
            <Col>
              <Card className="shadow border-0 rounded">
                <CardBody>
                  ...
                  <h5 className="card-title mt-4">{data?.whatIsADuct.title}</h5>
                  <p className="text-muted">{data?.whatIsADuct.text1}</p>
                  <p className="text-muted">{data?.whatIsADuct.text2}</p>

                  <h5 className="card-title mt-4">{data?.components.title}</h5>
                  <p className="text-muted">{data?.components.preventiveMaintenance.description}</p>
                  {data?.components.preventiveMaintenance.types.map((type, index) => (
                    <p className="text-muted mb-0" key={index}>{index + 1} {type}</p>
                  ))}

                  <h5 className="card-title mt-4">{data?.typesOfDuct.title}</h5>
                  {data?.typesOfDuct.tasks.map((task, index) => (
                    <p className="text-muted mb-0" key={index}>{task}</p>
                  ))}

                  <h5 className="card-title mt-4">{data?.materialsManufacturedForDuct.title}</h5>
                  <p className="text-muted">{data?.materialsManufacturedForDuct.text}</p>

                  <h5 className="card-title mt-4">{data?.thermal.title}</h5>
                  <p className="text-muted">{data?.thermal.text}</p>

                  <h5 className="card-title mt-4">{data?.ductDesign.title}</h5>
                  <p className="text-muted">{data?.ductDesign.text1}</p>
                  <p className="text-muted">{data?.ductDesign.text2}</p>

                  <h5 className="card-title mt-4">{data?.effect.title}</h5>
                  {data?.effect.points.map((point, index) => (
